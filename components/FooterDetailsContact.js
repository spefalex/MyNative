...
.catch((err) => { console.log(err); });
  ...
  <List style={{flexDirection:'column', flexWrap: 'wrap', alignItems: 'flex-start' ,flexDirection:'row'}} >
            ...
            <ListItem itemHeader>
    <Text style={{fontSize:16,color:"#ff6666"}} onPress={function(){ this.ignorer(this.props.idUserMaitre,this.props.idUserIgnorer) }.bind(this) } > <MyIcon name="close" size = {16} color="#ff6666" /> Retirer </Text>
            </ListItem>
  
            <ListItem>
  <Text style={{fontSize:16,color:'#00ffaa'}} onPress={function(){ this.sauvegarder(this.props.idUser,this.props.id) }.bind(this) }> <MyIcon name="wechat" size = {16} color="#00ffaa" /> Chat</Text>
            </ListItem>
